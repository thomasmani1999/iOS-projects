...
struct Movie : Codable,Identifiable {
    ...
    //Filter for data entered in search bar
    public func contains(searchText:String) -> Bool{
        var isIncluded = false
        if !isIncluded ,let movieTitle = title{
            isIncluded = movieTitle.contains(searchText)
        }
        if !isIncluded, let movieGenre = genre{
            isIncluded = movieGenre.contains(searchText)
        }
        if !isIncluded, let movieActors = actors{
            isIncluded = movieActors.contains(searchText)
        }
        if !isIncluded, let movieDirectors = director{
            isIncluded = movieDirectors.contains(searchText)
        }
        return isIncluded
    }
    
    //Filter function for menu filters
    public func filter(string:String) -> Bool{
        var isIncluded = false
        if !isIncluded ,let movieYear = year{
            isIncluded = movieYear.contains(string)
        }
        if !isIncluded, let movieGenre = genre{
            isIncluded = movieGenre.contains(string)
        }
        if !isIncluded, let movieActors = actors{
            isIncluded = movieActors.contains(string)
        }
        if !isIncluded, let movieDirectors = director{
            isIncluded = movieDirectors.contains(string)
        }
        return isIncluded
    }
}
